...
from model_utils import add_se_model, add_pc_loss
...
def SaveModel(args, train_model, epoch):
    prefix = "RETRAIN_[]_{}".format(train_model._device_prefix, train_model._devices[0])
...
def load_init_net_multigpu(args):
    # load pretrained init_net into CPU
    init_net_pb = args.init_net_pb
    init_net_proto = caffe2_pb2.NetDef()
    pretrained_blobs = []
    with open(init_net_pb, 'rb') as f:
        init_net_proto.ParseFromString(f.read())
        for op in init_net_proto.op:
            pretrained_blobs.append(op.output[0])
    workspace.RunNetOnce(core.Net(init_net_proto))

    # copy blobs to GPU
    assert(args.gpus is not None)
    gpus = [int(x) for x in args.gpus.split(',')]
    for blob in pretrained_blobs:
        blob_values = workspace.FetchBlob(blob)
        for gpu_id in gpus:
            prefix = "gpu_{}/".format(gpu_id)
            gpu_blob = prefix + blob
            device_opt = core.DeviceOption(device_type=caffe2_pb2.CUDA,
                                           cuda_gpu_id=gpu_id)
            workspace.FeedBlob(gpu_blob, blob_values, device_option=device_opt)
    log.info("Loading pretrained param_init_net to gpu over")
...
def Train(args):
    ...
    expname = "log/{}/RETRAIN_resnet50_gpu{}_b{}_L{}_lr{:.2f}_v2".format(
        args.dataset_name,
        args.num_gpus,
        total_batch_size,
        args.num_labels,
...
def parse_args():
    ...
    config_parser = argparse.ArgumentParser(
        description='Caffe2: Resnet-50 multi-gpu training config parser',
        add_help=False,
    )
    config_parser.add_argument('--multigpu_config', type=str, required=True,
                               help = 'multigpu training config file')
    args, _ = config_parser.parse_known_args()
    with open(args.multigpu_config) as f:
        config = yaml.load(f)
        config_parser.set_defaults(**config)

    # parse rest training specific arguments
    args_parser = argparse.ArgumentParser(
        description='Caffe2: Resnet-50 multi-gpu training args parser',
        parents=[config_parser]
    )
    args_parser.add_argument("--model_config", type=str, required=True,
                             help="config file for constructing model")
